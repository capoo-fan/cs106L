 ...
 */
template <typename K, typename M, typename H = std::hash<K>> class HashMap
{
    public:
    /*
    ...
    HashMap(const HashMap &other);                // 复制析构函数
    HashMap &operator=(const HashMap &other);     // 复制赋值运算符
    HashMap(HashMap &&other) noexcept;            // 移动构造函数
    HashMap &operator=(HashMap &&other) noexcept; // 移动赋值运算符
