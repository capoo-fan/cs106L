...
const std::vector<std::pair<std::string, int>> vec{{"A", 3},  {"B", 2},        {"C", 1},       {"A", -5},
                                                   {"B", 3},  {"A", 5},        {"C", 1},       {"D", 10},
                                                   {"K", 30}, {"Avery", 2020}, {"Anna", 2020}, {"Ali", 2019}};
...
template <typename... Ts> void ensure_at_return_is_const(Ts &...)
{
}

template <typename T> auto ensure_at_return_is_const(const T &map) -> decltype((void)(map.at(0) = 3), void())
{
...
void G_custom_hash_function()
{
    ...
    using K = int;
    using V = char;
    auto identity_shifted = [](const K &key) { return (key * 43037 + 52081) % 79229; };
...
void J_iter_foreach_one_bucket()
{
     ...
     */
    std::set<std::pair<int, int>> questions{{1, 1}, {2, 2}, {30, 30}, {140, 140}, {21, 21}};
...
void M_iter_foreach_edge()
{
    ...
    std::set<std::pair<int, int>> questions{{6, 6}, {1, 1}, {2, 2}, {3, 3}, {4, 4}};
...
void N_iter_forward_operators()
{
    ...
    std::set<std::pair<int, int>> questions{{1, 1}, {2, 4}, {3, 9}};
...
void P_iter_algorithm_permutation()
{
    ...
    auto identity_shifted = [](const int &key) { return (key * 43037 + 52081) % 79229; };
...
void Q_iter_const_iterator()
{
    ...
    std::set<std::pair<int, int>> questions{{1, 1}, {2, 2}, {3, 3}};
...
void A_range_ctor_basic()
{
    ...
    std::vector<std::pair<std::string, int>> values{{"Ignore me", 100}, {"A", 3}, {"B", 2}, {"C", 1},
                                                    {"A", -5},          {"B", 3}, {"A", 5}, {"C", 1}};
    ...
    VERIFY_TRUE(check_map_equal(map, answer),
                __LINE__); // maps should be equal, if not, you didn't transfer all the elements!
}
#endif
...
{
    /* Simple test of the range ctor taking in two iterators to another collection */
    std::vector<std::pair<std::string, int>> values{{"Ignore me", 100}};
    {
...
void C_initializer_list_ctor_basic()
{
    ...
    std::unordered_map<std::string, int> answer{{"A", 3}, {"B", 2}, {"C", 1}, {"A", -5}, {"B", 3}, {"A", 5}, {"C", 1}};

    HashMap<std::string, int> map{{"A", 3}, {"B", 2}, {"C", 1}, {"A", -5}, {"B", 3}, {"A", 5}, {"C", 1}};
...
#if RUN_TEST_3B
template <typename... Ts> void ensure_no_const_index_op(Ts &...)
{
}

template <typename T> auto ensure_no_const_index_op(const T &map) -> decltype((void)(map[0]), void())
{
    VERIFY_TRUE(false, __LINE__);
...
int A_benchmark_insert_erase()
{
    ...
    auto good_hash_function = [](const int &key) { return (key * 43037 + 52081) % 79229; };
    ...
    VERIFY_TRUE(10 * my_map_timing[0] < my_map_timing[3],
                __LINE__); // Ensure runtime of N = 10 is much faster than N = 10000
    return true;
}
...
template <typename T> int run_test(const T &test, const string &test_name)
{
    try
    {
        test();
    ...
    catch (const VerifyTrueAssertionFailure &e)
    {
        cout << "Test " << std::setw(30) << left << test_name << right
             << " FAIL: VERIFY_TRUE assertion failure at line number " << e.line << " in file tests.cpp" << endl;
    }
    catch (const std::exception &e)
    {
        cout << "Test " << std::setw(30) << left << test_name << right << " FAIL: exception thrown - " << e.what()
             << endl;
    }
    catch (const std::string &e)
    {
        cout << "Test " << std::setw(30) << left << test_name << right << " FAIL with string " << e << endl;
    }
    catch (const char *&e)
    {
        cout << "Test " << std::setw(30) << left << test_name << right << " FAIL with string " << e << endl;
